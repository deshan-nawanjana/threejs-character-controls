import { CharacterControls } from "./assets/modules/CharacterControls.js"
import { InputListener } from "./assets/modules/InputListener.js"

// model selector
const select = document.querySelector("select")
// get current model
const model = new URLSearchParams(window.location.search)
  .get("model") || "character_1"
// set model for selector
select.value = model
// switch character model
select.addEventListener("input", () => {
  window.location.href = "?model=" + select.value
})

// three modules
const scene = new THREE.Scene()
const renderer = new THREE.WebGLRenderer({ antialias: true })
const camera = new THREE.PerspectiveCamera(45, 0.6, 0.2, 500)
...
// get character model source path
const path = `./assets/characters/${model}/scene.fbx`
...
new THREE.FBXLoader().load(path, model => {
  ...
  // set ready attriute
  setTimeout(() => element.setAttribute("data-ready", "true"), 1000)
